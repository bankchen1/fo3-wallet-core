//! Raydium DEX integration for Solana
//!
//! This module provides functionality for interacting with the Raydium DEX on Solana,
...
/// Raydium client for interacting with the Raydium DEX
pub struct RaydiumClient {
    /// RPC client
    client: RpcClient,
    /// Pools cache
    pools: HashMap<String, RaydiumPool>,
}
...
impl RaydiumClient {
    ...
    pub fn init_pools(&mut self, pools: Vec<RaydiumPool>) {
        for pool in pools {
    ...
    /// Get all pools
    pub fn get_pools(&self) -> Vec<RaydiumPool> {
        self.pools.values().cloned().collect()
    }

    /// Find a pool by token mints
    pub fn find_pool(&self, token_a_mint: &str, token_b_mint: &str) -> Option<RaydiumPool> {
...
/// Load known Raydium pools from a JSON file
pub fn load_known_pools() -> Vec<RaydiumPool> {
    ...
    vec![
        ...
        RaydiumPool {
            ...
            pool_token_mint: "8HoQnePLqPj4M7PUDzfw8e3Ymdwgc7NLGnaTUapubyvu".to_string(),
