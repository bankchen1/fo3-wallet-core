...
// We'll use the base58 crate directly
use base58;
use bitcoin::Network;
...
pub fn public_key_to_address(public_key: &PublicKey, network: Network) -> Result<String> {
    ...
    let hash = bitcoin::hashes::sha256::Hash::hash(&public_key.serialize());
    let hash = bitcoin::hashes::ripemd160::Hash::hash(&hash[..]);
    ...
    // Add checksum
    let checksum = bitcoin::hashes::sha256::Hash::hash(&address);
    let checksum = bitcoin::hashes::sha256::Hash::hash(&checksum[..]);
    address.extend_from_slice(&checksum[0..4]);

    // Encode as base58
    Ok(base58::encode(&address))
}
